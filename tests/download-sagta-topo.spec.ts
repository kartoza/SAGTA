...
// Download Map tests for sagta_topo Project

import { test, expect } from '@playwright/test';

test.describe('Download Maps - sagta_topo Project', () => {
  ...
  test('Download sagta_topo - A3 Landscape Layout', async ({ page, context }) => {
    // 1. Navigate to sagta_topo project
    await login(page);
    const sagta_topoProject = page.locator('text=sagta_topo').first();
    await sagta_topoProject.click();
    await page.waitForLoadState('networkidle');
  ...
  test('Download sagta_topo - A3 Portrait Layout', async ({ page, context }) => {
    await login(page);
    const sagta_topoProject = page.locator('text=sagta_topo').first();
    await sagta_topoProject.click();
    await page.waitForLoadState('networkidle');
  ...
  test('Download sagta_topo - A4 Landscape Layout', async ({ page, context }) => {
    await login(page);
    const sagta_topoProject = page.locator('text=sagta_topo').first();
    await sagta_topoProject.click();
    await page.waitForLoadState('networkidle');
  ...
  test('Download sagta_topo - A4 Portrait Layout', async ({ page, context }) => {
    await login(page);
    const sagta_topoProject = page.locator('text=sagta_topo').first();
    await sagta_topoProject.click();
    await page.waitForLoadState('networkidle');
