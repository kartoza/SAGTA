...
// Layer Activation and Deactivation tests for sagta_hybrid Project

import { test, expect } from '@playwright/test';

test.describe('Layer Activation and Deactivation - sagta_hybrid Project', () => {
  ...
  test('Activate All Layers in sagta_hybrid', async ({ page }) => {
    // Precondition: User is logged in and viewing sagta_hybrid project
    await login(page);
  ...
  test('Deactivate All Layers in sagta_hybrid', async ({ page }) => {
    // Precondition: User is logged in and viewing sagta_hybrid project
    await login(page);
  ...
  test('Toggle Individual Layers in sagta_hybrid', async ({ page }) => {
    // Precondition: User is logged in and viewing sagta_hybrid project
    await login(page);
    ...
    const layerPanel = page.locator('[data-testid="layers"], .layers-panel, .layer-panel, .sidebar').first();
    const firstLayer = layerPanel.locator('.layer-item, [role="treeitem"], .layer, li:has-text("Layer")').first();
    const firstLayerCheckbox = firstLayer.locator('input[type="checkbox"], button[aria-label*="toggle"], .eye-icon').first();
    ...
    const layerItems = page.locator('.layer-item, [role="treeitem"], .layer, li:has-text("Layer")');
    const layerCount = await layerItems.count();
    const layersToTest = Math.min(5, Math.max(3, layerCount - 1));
